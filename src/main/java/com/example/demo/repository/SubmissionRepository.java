...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    ...
    @Query("SELECT s FROM Submission s WHERE (:user IS NULL OR s.user = :user) AND (:contestProblem IS NULL OR s.contestProblem = :contestProblem) AND (:status IS NULL OR s.status = :status) order by s.id desc")
